...
// Helper: Build cart badge HTML
function buildCartBadgeHTML(qty) {
    if (qty === 0) return '';
    ...
    grid.innerHTML = list.map(p => {
        ...
        <div class="product-card">
            ...
            <div class="product-info">
                ...
                <button class="add-to-cart" onclick="addToCart(${p.id}, event)" data-id="${p.id}"><span class="add-to-cart-badge">Add to Cart${buildCartBadgeHTML(qtyInCart)}</span></button>
            </div>
        </div>
    `}).join(""); 
}
...
function addToCart(id, event) { 
    ...
    // Update button badge
    const newQty = cart.find(i => i.id === id)?.quantity || 0;
    const btn = document.querySelector(`.add-to-cart[data-id="${id}"]`);
    if (btn) {
        btn.querySelector('.add-to-cart-badge').innerHTML = 'Add to Cart' + buildCartBadgeHTML(newQty);
    ...
    if (event && event.target) {
        const btnEl = event.target.closest('.add-to-cart');
        if (btnEl) {
...
function removeFromCart(id) { 
    ...
    saveCart(); 
    updateCart();
    // Update button badge on product card
    const btn = document.querySelector(`.add-to-cart[data-id="${id}"]`);
    if (btn) {
        btn.querySelector('.add-to-cart-badge').innerHTML = 'Add to Cart';
    }
}
