...
export async function onRequestPost(context) {
    ...
    try {
        ...
        try { await DB.prepare("ALTER TABLE products ADD COLUMN blurb TEXT DEFAULT ''").run(); } catch(e) { /* already exists */ }
        ...
        if (action === 'add') {
            const result = await DB.prepare(`
                INSERT INTO products (
                    ...
                    material, materialAr, pairings, collection_name, blurb
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `).bind(
                ...
                data.pairings || '', (data.collectionName || '').trim(), data.blurb || ''
            ).run();
        ...
        if (action === 'update' && id) {
            await DB.prepare(`
                UPDATE products SET
                    ...
                    pairings = ?, collection_name = ?, blurb = ?
                WHERE id = ?
            `).bind(
                data.slug, data.name, data.nameAr || '',
                data.category || '', data.categoryAr || '',
