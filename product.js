...
function resetToAddButton(productId) {
  ...
  document.querySelectorAll(`[id="transformedBtn-${productId}"]`).forEach(transformed => {
    ...
    if (isEarlyPrice) {
      ...
      transformed.outerHTML = `<button class="${btnClass}" id="${btnId}"><span class="btn-en">Add to Cart</span><span class="btn-ar arabic-text">أضف إلى السلة</span></button>`;
    }
  });
...
function renderVariantSelector(containerId, product, isMobile) {
  ...
  let tilesHTML = product.variants.map(v => {
    ...
    const zoomBtn = v.image
      ? `<button class="variant-img-zoom-btn" aria-label="Zoom image" onclick="event.stopPropagation();openVariantImagePopup('${v.image}','${escapedName}')"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg></button>`
      : '';
