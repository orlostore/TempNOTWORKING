...
// Helper: Get quantity in cart for a product
function getQtyInCartProduct(productId) {
    const localCart = JSON.parse(localStorage.getItem("cart")) || [];
    const item = localCart.find(i => i.id === productId);
    return item ? item.quantity : 0;
}

// Helper: Build cart badge HTML for product page
function buildProductBadgeHTML(qty) {
    if (qty === 0) return '';
    const twoDigit = qty >= 10 ? ' two-digit' : '';
    return `<span class="btn-cart-icon"><svg viewBox="0 0 24 24"><path d="M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zM1 2v2h2l3.6 7.59-1.35 2.45c-.16.28-.25.61-.25.96 0 1.1.9 2 2 2h12v-2H7.42c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63h7.45c.75 0 1.41-.41 1.75-1.03l3.58-6.49c.08-.14.12-.31.12-.48 0-.55-.45-1-1-1H5.21l-.94-2H1zm16 16c-1.1 0-1.99.9-1.99 2s.89 2 1.99 2 2-.9 2-2-.9-2-2-2z"/></svg><span class="btn-qty-badge${twoDigit}">${qty}</span></span>`;
}
...
async function initProductPage() {
  ...
  // =====================
  // ADD TO CART BUTTONS with badge
  // =====================
  const qtyInCart = getQtyInCartProduct(product.id);
  ...
  const addToCartHandler = () => {
    ...
    // Update buttons with new badge
    const newQty = localCart.find(i => i.id === product.id)?.quantity || 0;
    updateProductPageButtons(newQty);
    
    return true;
  };
  
  // Function to update both buttons
  const updateProductPageButtons = (qty) => {
    const badgeHTML = buildProductBadgeHTML(qty);
    const desktopBtn = document.getElementById("addToCartBtn");
    const mobileBtn = document.getElementById("mobileAddToCartBtn");
    
    if (desktopBtn) {
      desktopBtn.innerHTML = `<span class="add-to-cart-badge">Add to Cart${badgeHTML}</span>`;
      const badge = desktopBtn.querySelector('.btn-qty-badge');
      if (badge) { badge.classList.add('pop'); setTimeout(() => badge.classList.remove('pop'), 300); }
    }
    if (mobileBtn) {
      mobileBtn.innerHTML = `<span class="add-to-cart-badge">Add to Cart | أضف إلى السلة${badgeHTML}</span>`;
      const badge = mobileBtn.querySelector('.btn-qty-badge');
      if (badge) { badge.classList.add('pop'); setTimeout(() => badge.classList.remove('pop'), 300); }
    }
  };
  
  // Set initial button content with badge
  const desktopBtn = document.getElementById("addToCartBtn");
  const mobileBtn = document.getElementById("mobileAddToCartBtn");
  
  if (desktopBtn) {
    desktopBtn.innerHTML = `<span class="add-to-cart-badge">Add to Cart${buildProductBadgeHTML(qtyInCart)}</span>`;
  }
  if (mobileBtn) {
    mobileBtn.innerHTML = `<span class="add-to-cart-badge">Add to Cart | أضف إلى السلة${buildProductBadgeHTML(qtyInCart)}</span>`;
  }
