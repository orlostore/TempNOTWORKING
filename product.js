...
async function initProductPage() {
  ...
  const shareText = encodeURIComponent(product.name + ' - ORLO Store');
  const shareHTML = `
    <div class="product-share-buttons">
      <span class="share-label">Share | <span class="arabic-text">شارك</span></span>
      <a href="https://wa.me/?text=${shareText}%20${encodeURIComponent(shareUrl)}" target="_blank" rel="noopener" class="share-btn share-whatsapp" aria-label="Share on WhatsApp">
      ...
      <button class="share-btn share-copy" onclick="copyProductLink()" aria-label="Copy link">
  ...
  window.copyProductLink = function() {
    navigator.clipboard.writeText(shareUrl).then(function() {
      ...
      if (copyBtn) { copyBtn.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>'; setTimeout(function() { copyBtn.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>'; }, 2000); }
    });
  };
