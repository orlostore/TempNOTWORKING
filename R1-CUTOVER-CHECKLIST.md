...
- [ ] **10. Reconcile mobile cart heights** — the overlay currently overrides `.cart-sidebar { height: calc(100vh - 85px) !important; }` because the base 70px was too tight (clipped the buttons against the nav). 85px matches the actual mobile bottom nav height with home-indicator safe area. The breathing room above the buttons comes from `bottom: 25px` on `.cart-checkout-fixed`, NOT from making the sidebar shorter (which would expose the page behind). Update the base rule in `styles.css`'s `@media (max-width:768px)` block to use 85px too. Drop the `!important`.
