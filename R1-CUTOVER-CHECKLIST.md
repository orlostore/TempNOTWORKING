...
- [ ] **10. Reconcile mobile cart heights** — the overlay currently overrides `.cart-sidebar { height: calc(100vh - 115px) !important; }` because the base 70px was too tight. Update the base rule in `styles.css`'s `@media (max-width:768px)` block to use 115px too. Drop the `!important`.
